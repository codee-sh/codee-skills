...
Generic front-end code lives under `src/components/`, domain-specific components live under
`src/modules/<module>/components/`, and routed pages live under
`src/app/[locale]/(frontend)/`. Styling is Tailwind with design tokens; every reusable class
string is centralised in `src/lib/class-names.ts`. Text goes through `next-intl`.
...
Is it tied to one business module (training, clients, sharing)?
        └── yes → src/modules/{module}/components/{feature}/

Is it a routed screen?
        └── yes → src/app/[locale]/(frontend)/{route}/page.tsx
```

Rule of thumb: if it imports a domain type (`TExercise`, `SetLog`, `MetricField`) or a
module API, it belongs to that module and does **not** belong in `ui/`.
...
### `modules/{module}/components/{feature}/` — main file + components + hooks

```
src/modules/{module}/components/{feature}/
...
Real example — `modules/training/components/exercise-card/`:
```
...
Real example — `modules/training/components/workout-plans/`:
```
workout-plans.tsx
index.ts
...
`ui`/`common`: `{name}/{name}.tsx` + `index.ts`. Module feature: add
`components/{name}/{name}.tsx` + `index.ts` in the existing feature folder, or create
`src/modules/{module}/components/{feature}/` with `{feature}.tsx` + `index.ts`.
...
### 3. Server page → module server API → client component

Pages are **server components**. They fetch through the owning module's explicit `server/`
entry point, then pass plain data to client components. Server entry points must import
`server-only`. Never query inside the JSX.
...
Server queries are plain async functions returning a typed result (e.g. a discriminated
union for auth state). Reuse `PageContainer` / `PageHeader` for every page shell.
...
Place it in `hooks/` inside the module feature:
`src/modules/{module}/components/{feature}/hooks/use-{feature}-{type}.ts`.

```ts
// modules/training/components/workout-tracker/hooks/use-workout-session.ts
'use client'
...
| Rule | Detail |
|---|---|
| Location first | `ui/` (generic) vs `common/` (shared app) vs `modules/{module}/components/{feature}/` (domain) |
...
| Pages = server + module query | Fetch through `modules/{module}/.../server`; render with `PageContainer`/`PageHeader`; never query in JSX |
