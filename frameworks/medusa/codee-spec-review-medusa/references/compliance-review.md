...
| Canonical mechanism reuse | Existing Medusa core endpoints/workflows and project primitives are preferred; every custom mechanism has a concrete gap and rationale. | `codee-medusa-backend`; current repository code | PASS / FAIL / N/A | |
| Layering and ownership | Module, workflow, route, event/job, and frontend responsibilities follow current project architecture; business logic is not placed in transport code. | `codee-medusa-backend` and routed references | PASS / FAIL / N/A | |
